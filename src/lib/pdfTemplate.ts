...
// Strip em dashes from LLM-generated content — mirrors noDash() in BriefRenderer
...
function sectionHeading(title: string): string {
  return `<div style="margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid ${C.border};">
    <span style="font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:700;color:${C.textPrimary};text-transform:uppercase;letter-spacing:0.04em;">${esc(title)}</span>
  </div>`;
}
...
export function generateBriefHtml(brief: VisaBrief, meta: PdfMeta): string {
  ...
  const recommendedAction = `<div style="border:1px solid ${C.border};border-top:3px solid ${C.amber};border-radius:8px;overflow:hidden;margin-bottom:16px;page-break-inside:avoid;">
    ...
    <div style="background:${C.bg};padding:16px;">
      ...
      <div style="display:flex;align-items:center;gap:8px;margin-top:12px;padding-top:10px;border-top:1px solid ${C.border};">
        <span style="font-family:'JetBrains Mono',monospace;font-size:9.5px;font-weight:700;text-transform:uppercase;letter-spacing:0.04em;padding:2px 8px;border-radius:4px;background:${confBg};color:${confColor};">CONF · ${esc(brief.confidenceScore.overall.toUpperCase())}</span>
        <span style="font-family:'JetBrains Mono',monospace;font-size:9.5px;color:${C.textTertiary};text-transform:uppercase;letter-spacing:0.04em;font-weight:700;">Overall Confidence</span>
      </div>
    </div>
  </div>`;

  const visaOptions = card('Visa Options', brief.visaOptions.map(opt => visaOptionHtml(opt, depth)).join(''));
  ...
  const borderContent = `
    ...
    ${label('Enforcement Posture')}<p style="font-size:12px;color:${C.textSecondary};margin:0 0 10px;">${esc(nd(brief.borderRunAnalysis.enforcementPosture))}</p>
    ${warningBox('Warnings', brief.borderRunAnalysis.warnings)}`;
  const borderRun = depth !== 'quick' ? card('Border Run Analysis', borderContent) : '';

  const recentChanges = brief.recentChanges.hasChanges ? card(
    'Recent Changes & Watch Items',
    `<ul style="list-style:none;padding:0;margin:0${brief.recentChanges.watchItems.length ? ' 0 10px' : ''};">${brief.recentChanges.items.map(i => `<li style="font-size:12px;color:${C.textSecondary};padding:3px 0;">• ${esc(nd(i))}</li>`).join('')}</ul>${warningBox('Watch Items', brief.recentChanges.watchItems)}`,
  ) : '';
...
<body>
  ...
  <div style="border-bottom:1px solid rgba(99,102,241,0.4);padding-bottom:14px;margin-bottom:18px;">
    <div style="display:flex;justify-content:space-between;align-items:flex-end;">
      ...
      <div style="text-align:right;">
        ${mono(esc((DEPTH_LABEL[depth as BriefDepth] ?? depth).toUpperCase()) + ' DEPTH', { size: 9.5, color: C.textTertiary, tracking: '0.04em' })}
        <br>
        ${mono('Generated ' + esc(generatedDate) + ' · ' + esc(generatedTime) + ' UTC', { size: 9, color: C.textTertiary, tracking: '0.04em' })}
      </div>
    </div>
  </div>
  ...
  <div style="page-break-inside:avoid;margin-top:16px;padding-top:12px;border-top:1px solid ${C.border};">
    ...
    <p style="font-family:'JetBrains Mono',monospace;font-size:9px;color:${C.textTertiary};text-transform:uppercase;letter-spacing:0.04em;text-align:center;">Generated by VisaScout · visascout.io · ${esc(generatedDate)} · ${esc(generatedTime)} UTC</p>
  </div>
</body>
</html>`;
}
